...
UTIL_TIMER_Status_t TIMER_IF_Init(void) {
    ...
    if (RTC_Initialized == false) {
        _rtc.IsEnabled.RtcFeatures = UINT32_MAX;
        /*Init RTC*/
        // MX_RTC_Init();
        /*Stop Timer */
        TIMER_IF_StopTimer();
...
UTIL_TIMER_Status_t TIMER_IF_StartTimer(uint32_t timeout) {
    ...
    if (HAL_RTC_SetAlarm_IT(&_rtc, &sAlarm, RTC_FORMAT_BCD) != HAL_OK) {
        // Error_Handler();
        LOG_ERROR("Timer HAL_RTC_SetAlarm_IT");
    }
...
void HAL_RTC_AlarmAEventCallback(RTC_HandleTypeDef *_rtc) {
    UTIL_TIMER_IRQ_MAP_PROCESS();
}

void HAL_RTCEx_SSRUEventCallback(RTC_HandleTypeDef *_rtc) {
...
void TIMER_IF_BkUp_Write_Seconds(uint32_t Seconds) {
    HAL_RTCEx_BKUPWrite(&_rtc, RTC_BKP_SECONDS, Seconds);
}

void TIMER_IF_BkUp_Write_SubSeconds(uint32_t SubSeconds) {
    HAL_RTCEx_BKUPWrite(&_rtc, RTC_BKP_SUBSECONDS, SubSeconds);
}

uint32_t TIMER_IF_BkUp_Read_Seconds(void) {
    uint32_t ret = 0;

    ret = HAL_RTCEx_BKUPRead(&_rtc, RTC_BKP_SECONDS);
...
uint32_t TIMER_IF_BkUp_Read_SubSeconds(void) {
    ...
    ret = HAL_RTCEx_BKUPRead(&_rtc, RTC_BKP_SUBSECONDS);
...
/* Private functions ---------------------------------------------------------*/
static void TIMER_IF_BkUp_Write_MSBticks(uint32_t MSBticks) {
    HAL_RTCEx_BKUPWrite(&_rtc, RTC_BKP_MSBTICKS, MSBticks);
}

static uint32_t TIMER_IF_BkUp_Read_MSBticks(void) {
    uint32_t MSBticks;
    MSBticks = HAL_RTCEx_BKUPRead(&_rtc, RTC_BKP_MSBTICKS);
    return MSBticks;
}

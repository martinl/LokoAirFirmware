...
static LmHandlerJoinParams_t JoinParams = { .CommissioningParams = &CommissioningParams, .Mode = ACTIVATION_TYPE_NONE, .forceRejoin = false, .Datarate = DR_0, .Status = LORAMAC_HANDLER_ERROR };
...
LmHandlerErrorStatus_t LmHandlerConfigure(LmHandlerParams_t *handlerParams) {
    ...
    memcpy((void *)&LmHandlerParams, (const void *)handlerParams, sizeof(LmHandlerParams_t));
            ...
            switch (notifyType) {
                ...
                case PACKAGE_MCPS_INDICATION: {
                    ...
                    if ((LmHandlerPackages[i]->OnMcpsIndicationProcess != NULL) &&
                        ((LmHandlerPackages[i]->Port == ((McpsIndication_t *)params)->Port) || ((i == PACKAGE_ID_COMPLIANCE) && (LmHandlerPackages[PACKAGE_ID_COMPLIANCE]->IsRunning()))))
#elif (defined(LORAMAC_VERSION) && (LORAMAC_VERSION == 0x01000400))
                    if (LmHandlerPackages[i]->OnMcpsIndicationProcess != NULL)
#endif /* LORAMAC_VERSION */
                    {
...
static void LmHandlerPackagesProcess(void) {
    for (int8_t i = 0; i < PKG_MAX_NUMBER; i++) {
        if ((LmHandlerPackages[i] != NULL) && (LmHandlerPackages[i]->Process != NULL) && (LmHandlerPackageIsInitialized(i) != false)) {
...
LmHandlerErrorStatus_t LmHandlerGetDevEUI(uint8_t *devEUI) {
    ...
    memcpy(devEUI, mibReq.Param.DevEui, SE_EUI_SIZE);
    return LORAMAC_HANDLER_SUCCESS;
}
...
LmHandlerErrorStatus_t LmHandlerGetAppEUI(uint8_t *appEUI) {
    ...
    memcpy(appEUI, mibReq.Param.JoinEui, SE_EUI_SIZE);
    return LORAMAC_HANDLER_SUCCESS;
}
...
LmHandlerErrorStatus_t LmHandlerGetNwkKey(uint8_t *nwkKey) {
    ...
    memcpy(nwkKey, keyItem->KeyValue, 16);
#else
...
LmHandlerErrorStatus_t LmHandlerGetAppKey(uint8_t *appKey) {
    ...
    memcpy(appKey, keyItem->KeyValue, 16);
#else
...
LmHandlerErrorStatus_t LmHandlerGetNwkSKey(uint8_t *nwkSKey) {
    ...
    memcpy(nwkSKey, keyItem->KeyValue, 16);
#else
...
LmHandlerErrorStatus_t LmHandlerGetAppSKey(uint8_t *appSKey) {
    ...
    memcpy(appSKey, keyItem->KeyValue, 16);
#else
